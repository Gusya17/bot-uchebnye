...
Таблица `orders` в `bot.db`: id, tg_id, tg_username, name, work_type, institution, faculty, specialization, course, study_form, topic, volume, uniqueness, deadline, materials (строка через запятую), phone, email, created_at.
...
В `cb_confirm_yes` (файл `order_handlers.py`), сразу после `save_full_order()`, бот отправляет администратору сообщение вида:
